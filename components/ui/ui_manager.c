...
static void home_create(void){s_home=lv_obj_create(NULL);base(s_home);gestures(s_home);lv_obj_t*w=card(s_home,12,10,170,152);label(w,LV_SYMBOL_EYE_OPEN,10,6,20);label(w,"26 C",10,35,28);label(w,"Cloudy",10,72,16);lv_obj_t*m=label(w,"Humidity 68%   AQI 32",10,110,14);lv_obj_set_style_text_color(m,lv_color_hex(MUTED),0);label(s_home,"SEP 24  THU",224,26,20);label(s_home,"12:53",220,58,28);lv_obj_t*h=label(s_home,"Swipe horizontally for functions",222,128,14);lv_obj_set_style_text_color(h,lv_color_hex(MUTED),0);}
static void app_btn(lv_obj_t*p,const char*i,const char*t,int x,lv_event_cb_t cb){lv_obj_t*b=card(p,x,38,142,112);lv_obj_add_flag(b,LV_OBJ_FLAG_CLICKABLE);lv_obj_add_event_cb(b,cb,LV_EVENT_CLICKED,NULL);lv_obj_t*ic=label(b,i,48,16,28);lv_obj_set_style_text_color(ic,lv_color_hex(BLUE),0);label(b,t,36,70,16);}
static void apps_create(void){s_apps=lv_obj_create(NULL);base(s_apps);gestures(s_apps);label(s_apps,"HOME",14,10,14);label(s_apps,"Functions",268,10,16);app_btn(s_apps,LV_SYMBOL_LIST,"Remote",16,go_remote);app_btn(s_apps,LV_SYMBOL_AUDIO,"Music",171,go_music);app_btn(s_apps,LV_SYMBOL_EYE_OPEN,"Lights",326,go_light);app_btn(s_apps,LV_SYMBOL_HOME,"Devices",481,go_devices);}
static void settings_create(void){s_settings=lv_obj_create(NULL);base(s_settings);gestures(s_settings);label(s_settings,"System settings",24,12,20);volume_slider=slider_row(s_settings,"Volume",52,60);brightness_slider=slider_row(s_settings,"Brightness",96,80);lv_obj_t*h=label(s_settings,"Swipe up to close",250,145,14);lv_obj_set_style_text_color(h,lv_color_hex(MUTED),0);}
static void light_create(void){s_light=lv_obj_create(NULL);base(s_light);gestures(s_light);label(s_light,"<",16,10,20);light_title=label(s_light,lights[0],54,12,20);light_switch=lv_switch_create(s_light);lv_obj_set_pos(light_switch,560,12);lv_obj_add_state(light_switch,LV_STATE_CHECKED);lv_obj_add_event_cb(light_switch,light_power_cb,LV_EVENT_VALUE_CHANGED,NULL);light_bri=slider_row(s_light,"Brightness",52,72);light_temp=slider_row(s_light,"Warm / Cool",88,58);const char*p[]={"Relax","Read","Movie","Night"};for(int i=0;i<4;i++)click_card(s_light,20+i*155,124,135,38,p[i],preset_cb,i);}
...
static void remote_create(void){s_remote=lv_obj_create(NULL);base(s_remote);gestures(s_remote);label(s_remote,"TV Remote",18,10,20);rbtn(274,8,92,34,LV_SYMBOL_UP,UI_REMOTE_UP);rbtn(176,48,92,LV_SYMBOL_LEFT,UI_REMOTE_LEFT);rbtn(274,48,92,"OK",UI_REMOTE_OK);rbtn(372,48,92,LV_SYMBOL_RIGHT,UI_REMOTE_RIGHT);rbtn(274,88,92,LV_SYMBOL_DOWN,UI_REMOTE_DOWN);rbtn(18,132,82,"Back",UI_REMOTE_BACK);rbtn(108,132,82,LV_SYMBOL_HOME,UI_REMOTE_HOME);rbtn(198,132,82,LV_SYMBOL_PLAY,UI_REMOTE_PLAY_PAUSE);rbtn(450,132,82,"Vol+",UI_REMOTE_VOL_UP);rbtn(540,132,82,LV_SYMBOL_POWER,UI_REMOTE_POWER);}
static void music_create(void){s_music=lv_obj_create(NULL);base(s_music);gestures(s_music);lv_obj_t*c=card(s_music,16,16,136,136);label(c,LV_SYMBOL_AUDIO,48,38,28);label(s_music,"Time Traveler",184,22,20);lv_obj_t*a=label(s_music,"Zhou Shen",184,50,14);lv_obj_set_style_text_color(a,lv_color_hex(MUTED),0);music_progress=lv_slider_create(s_music);lv_obj_set_pos(music_progress,184,82);lv_obj_set_size(music_progress,420,10);lv_slider_set_value(music_progress,36,LV_ANIM_OFF);lv_obj_add_event_cb(music_progress,slider_cb,LV_EVENT_VALUE_CHANGED,NULL);click_card(s_music,316,112,54,46,LV_SYMBOL_PREV,music_cb,UI_MUSIC_PREV);lv_obj_t*b=click_card(s_music,382,108,62,54,"",music_cb,UI_MUSIC_PLAY_PAUSE);music_play_label=label(b,LV_SYMBOL_PAUSE,12,7,20);click_card(s_music,456,112,54,46,LV_SYMBOL_NEXT,music_cb,UI_MUSIC_NEXT);}
static void devices_create(void){s_devices=lv_obj_create(NULL);base(s_devices);gestures(s_devices);label(s_devices,"Devices",24,12,20);label(s_devices,LV_SYMBOL_HOME,302,42,28);label(s_devices,"No devices yet",258,82,16);lv_obj_t*t=label(s_devices,"Reserved for HA / robot devices",202,118,14);lv_obj_set_style_text_color(t,lv_color_hex(MUTED),0);}
