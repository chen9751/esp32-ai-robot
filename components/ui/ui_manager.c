...
static bool screen_load_pending;
static void load_async_cb(void *unused){
 (void)unused;
 lv_obj_t *s=pending_screen;
 pending_screen=NULL;
 screen_load_pending=false;
 if(s&&lv_screen_active()!=s)lv_screen_load(s);
}
static void load(lv_obj_t*s){
 if(!s||lv_screen_active()==s)return;
 pending_screen=s;
 if(!screen_load_pending){
  screen_load_pending=true;
  lv_async_call(load_async_cb,NULL);
 }
}
