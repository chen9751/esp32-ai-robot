...
static void menu_item_clicked(lv_event_t *e)
{
    ...
    if (item != NULL && s_action_cb != NULL) s_action_cb(item->action, s_action_user_data);
}
...
static void style_menu_label(lv_obj_t *label)
{
    ...
    if (s_menu_font != NULL) lv_obj_set_style_text_font(label, s_menu_font, 0);
    else lv_obj_set_style_text_font(label, &lv_font_montserrat_16, 0);
}
...
void ui_set_menu_action_cb(ui_menu_action_cb_t cb, void *user_data) { s_action_cb = cb; s_action_user_data = user_data; }
void ui_set_menu_font(const lv_font_t *font) { s_menu_font = font; }
...
void ui_show_main_menu(void)
{
    ...
    for (size_t i = 0; i < sizeof(MENU_ITEMS) / sizeof(MENU_ITEMS[0]); ++i) create_menu_item(scroller, &MENU_ITEMS[i]);
}

void ui_init(void) { ui_show_main_menu(); }
